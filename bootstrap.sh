...
# can build and install the Debian package for the 'gpsoauth'
# Python library (see https://github.com/simon-weber/gpsoauth)
...
if [[ "$#" -eq 0 || "$#" -gt 1 ]]; then
    echo "Please provide the gpsoauth's git tag to be packaged."
    echo "e.g.: $0 0.0.4"
    exit 1
fi
...
# Clone or update the gpsoauth repo
echo
echo "-----------------------------------------------------------"
echo "Cloning/updating gpsoauth's repo"
echo "-----------------------------------------------------------"
echo
if [[ ! -e "gpsoauth" ]]; then
    git clone https://github.com/simon-weber/gpsoauth/ && cd gpsoauth
    exit_code=$?
else
    echo "gpsoauth clone exists; checking for changes."
    cd gpsoauth && git pull origin master
    exit_code=$?
fi

if [[ $exit_code -ne 0 ]]; then
    echo "Error while cloning/updating the gpsoauth repo."
    exit 1
fi
...
# Patch known issues in gpsoauth sources
if [[ "$tag" == "0.0.4" ]]; then
...
echo
echo "Done configuring gpsoauth-deb."
